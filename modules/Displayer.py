...
def populate_table(i, key, table, currency):
    ...
    if float(key['percent_change_24h']) < 0:
        row.append(Colours.RED + f"{key['percent_chanage_24h']}%" + Colours.END)
    else: 
        row.append(Colours.GREEN + f"{key['percent_chanage_24h']}%" + Colours.END)
    if float(key['percent_change_7d']) < 0:
        row.append(Colours.RED + f"{key['percent_chanage_7d']}%" + Colours.END)
    else:
        row.append(Colours.GREEN + f"{key['percent_chanage_24h']}%" + Colours.END)
    table.append(row)
...
def display_information(data, currency, find):
    headers=["Rank", "Coin", f"Price ({currency.upper()})", f"Market Cap ({currency.upper()})", "Change (24H)","Change (7d)" ]
